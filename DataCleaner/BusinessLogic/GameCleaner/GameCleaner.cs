...
using DataCleaner.BusinessLogic.Mappers;

namespace DataCleaner.BusinessLogic.GameCleaner
{
	public class GameCleaner
	{
        ...
        public async Task CleanGamesInSeasons(YearRange seasonYearRange)
		{
            ...
            {
                var games = await _gameRepo.GetRichSeasonGames(seasonStartYear);
                ...
                if(seasonStartYear != seasonYearRange.EndYear)
                    gamesToClean = GetGamesToClean(existingCleanedGames[seasonStartYear], games);
        ...
        private async Task<IEnumerable<DbCleanedGame>> CleanGames(IEnumerable<Game> gamesToClean)
        {
            ...
            var seasonGames = await _gameRepo.GetRichSeasonGames(seasonStartYear);
            var lastSeasonGames = await _gameRepo.GetRichSeasonGames(seasonStartYear - 1);
